...
class TensorflowBenchmark:
    ...
    def run_benchmark(self):
        ...
        with mlflow.start_run(run_name="TensorFlow_Keras_MLP"):
            ...
            tf_callback = tf.keras.callbacks.TensorBoard(log_dir="../logs/tensorflow_mlp", histogram_freq=1)
            # Zwiększamy liczbę epok dla dłuższego, dokładniejszego treningu
            model.fit(X_train, y_train, epochs=30, batch_size=32, validation_split=0.1, verbose=0, callbacks=[tf_callback])
            ...
            model.save(os.path.join(models_dir, "census_tf_model.keras"))
            joblib.dump(self.scaler, os.path.join(models_dir, "census_tf_scaler.pkl"))
            print(f"💾 Model TensorFlow oraz Skaler zostały pomyślnie zapisane w: {models_dir}")
